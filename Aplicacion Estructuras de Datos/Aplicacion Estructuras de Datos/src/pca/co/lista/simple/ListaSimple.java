...
public class ListaSimple {
    ...
    public void actualizar(int dato){
        ...
        while(p.getEnlace()!=null&&encontro==false){
            if(p.getInfo()==dato){
                valorNuevo=Integer.parseInt(JOptionPane.showInputDialog(null, "Ingrese el nuevo valor"));
                p.setInfo(valorNuevo);
                encontro=true;
            }
            p=p.getEnlace();
        }
        if (buscarNodo(dato)) {
            JOptionPane.showMessageDialog(null,"Se actualizó el valor "+dato);
        } else {
